...
Main app code lives under `app/` using the Next.js App Router; feature folders like `bookmarks/` pair page entry points with co-located UI, and `[section]/` generates every section page from `constants/sections.ts`. Shared components sit in `components/` (Shadcn UI primitives in `components/ui/`). Cross-cutting constants, contexts, and hooks live in `constants/`, `contexts/`, and `hooks/`. Drop reusable utilities into `lib/`. Static assets belong in `public/`, and long-form docs stay in `docs/`. Add new tests next to the file they cover (e.g. `app/foo/Button.test.tsx`).
